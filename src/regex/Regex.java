...
class Regex {
    public static void main(String[] args) {
        ...
        System.out.println("The following pattern ('\\d'){36} matches : 36 digits in a row. \\d matches any digit and {36} looks for 36 matches in a row.");



        String[] TEKmentors = {"Amir Yunas","Mark Bennett", "Rosa Garcia", "Desaree Byers", "Abram Jablonski", "Dylan Fellows", "Emilios Papas", "Jonathan Diamond"};
        ...
        tekMentors = Pattern.compile("\\w+[\\^\\s]")
                .matcher(Arrays.toString(TEKmentors))
                .results()
        ...
        Pattern bookPattern = Pattern.compile("[B|b]ook");
        Matcher bookmatcher = bookPattern.matcher(bookText);
        bookCount = bookmatcher.results().count();
        System.out.println("'book' found in bookText " + bookCount + " times");
        ...
        String sansSleepy = Pattern.compile("[S|s]leepy").matcher(sleepy).replaceAll("@");
        String[] awake = Pattern.compile("[a-zA-Z,'.]+")
                .matcher(sansSleepy)
                .results()
                .map(MatchResult::group)
                .toArray(String[]::new);
        ...
        String[] arrows = {"\u21FD", "\u26F7", "\u21FF", "\u21EF","\u21EC", "\u26F9","\u26FD","\u26D4","\u26A5","\u21FD","\u2190", "\u26A1","\u21BA","\u2196","\u2603","\u21FD"};
        String[] onlyArrows = Pattern.compile("\\p{InArrows}")
                .matcher(Arrays.toString(arrows))
                .results()
                .map(MatchResult::group)
                .toArray(String[]::new);
        System.out.println(Arrays.toString(onlyArrows));
    }
